@php
$isRtl = in_array(app()->getLocale(), ['ar', 'ckb']);
@endphp
<header class="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/80 backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/80">
    <div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between {{ $isRtl ? 'flex-row-reverse' : '' }}">
            <!-- Logo -->
            <div class="flex items-center {{ $isRtl ? 'gap-8 flex-row-reverse' : 'gap-8' }}">
                <a href="/docs" class="flex items-center {{ $isRtl ? 'gap-3 flex-row-reverse' : 'gap-3' }} text-gray-900 dark:text-white transition-colors hover:text-orange-600 dark:hover:text-orange-400">
                    <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-orange-500 to-red-600 text-white font-bold text-sm shadow-sm">
                        {{ config('app.name')[0] ?? 'J' }}
                    </div>
            ...
            <!-- Search -->
            <div class="relative flex-1 max-w-lg {{ $isRtl ? 'mx-8' : 'mx-8' }}">
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full rounded-lg border border-gray-300 bg-gray-50/50 {{ $isRtl ? 'pr-10 pl-4' : 'pl-10 pr-4' }} py-2.5 text-sm placeholder-gray-500 outline-none transition-all duration-200 focus:border-orange-500 focus:bg-white focus:ring-2 focus:ring-orange-500/20 dark:border-gray-700 dark:bg-gray-900/50 dark:text-white dark:placeholder-gray-400 dark:focus:border-orange-400 dark:focus:bg-gray-900 dark:focus:ring-orange-400/20"
                        autocomplete="off"
                        dir="{{ $isRtl ? 'rtl' : 'ltr' }}"
                    />
                    <div class="absolute inset-y-0 {{ $isRtl ? 'left-0 pl-3' : 'right-0 pr-3' }} flex items-center pointer-events-none">
                        <kbd class="hidden sm:inline-flex items-center rounded border border-gray-200 px-1.5 py-0.5 text-xs font-mono text-gray-500 dark:border-gray-700 dark:text-gray-400">
                            ⌘K
                        </kbd>
                    </div>
                </div>
                <div id="docs-search-results" class="absolute top-full mt-2 hidden max-h-96 w-full overflow-auto rounded-lg border border-gray-200 bg-white p-2 shadow-xl dark:border-gray-700 dark:bg-gray-900 {{ $isRtl ? 'text-right' : 'text-left' }}">
