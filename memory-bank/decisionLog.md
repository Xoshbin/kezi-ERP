...
[2025-07-26 22:00:21] - **Decision:** Refactored the `checkIfPeriodIsLocked` method from `JournalEntryService` into a new, dedicated `LockDateService`.
...
- **Implications:** All services requiring this validation (`JournalEntryService`, `VendorBillService`, etc.) must now inject `LockDateService`. All service instantiations throughout the app (including tests and UI components like Filament pages) must be updated to use the service container (`app(...)`) to handle dependency injection correctly.
...
[2025-08-06 06:55:00] - **Decision:** Adopted the "Explicit Context Pattern" as a definitive solution for framework dependency issues during model creation.
- **Rationale:** To solve issues where a model's attribute (e.g., one handled by a cast) depends on context from a parent model that isn't available during Eloquent's creation lifecycle. This pattern shifts the responsibility of creating context-aware objects (like `Brick\Money\Money`) to the calling code (Actions or Services), making the creation process explicit and predictable.
- **Implementation Details:** The calling code must now create a complete, context-aware object and pass it to the model's creation method, rather than passing a raw value and relying on the model to infer context.

[2025-08-06 22:58:07] - **Decision:** The `CreateJournalEntryForPaymentAction` must always create journal entries in the company's base currency. All payments made in a foreign currency must be converted to the base currency before the journal entry is created. This ensures consistency and prevents `MoneyMismatchException` errors.
[2025-08-06 22:58:07] - **Decision:** Validation for balanced journal entries (debits must equal credits) is the responsibility of the `CreateJournalEntryAction`. Tests that check for unbalanced entries must assert the `ValidationException` on the Action itself, not on services that call the action. This centralizes the validation logic and makes testing more precise.
[2025-08-07 09:42:13] - [Filament Repeater Field Testing]
**Decision:** When testing Filament repeater fields, do not assume a zero-based index for line items. Instead, dynamically retrieve the keys of the repeater items and use them in assertions.
**Rationale:** Repeater fields in Filament use UUIDs as keys for each item, not a sequential index. Hardcoding an index like `lines.0.debit` will fail if the underlying implementation changes or in cases where the key is not a simple integer. The correct approach is to get the form data, find the key of the desired item, and then use that key to construct the assertion. This makes the test more robust and less likely to break due to internal implementation details of the repeater field.
[2025-08-07 12:51:52] - **Decision:** Refactored `mutateFormDataBeforeFill` in `EditJournalEntry.php` to use `Money` object arithmetic (`plus`, `minus`) for calculating totals, instead of casting `BigDecimal` objects to floats.
**Rationale:** Direct casting of the `BigDecimal` object returned by `$money->getAmount()` caused a fatal `ErrorException`. The correct, type-safe, and precision-preserving method is to use the arithmetic methods provided by the `Money` object itself.
**Implications:** This establishes a mandatory pattern for all financial aggregations. The codebase must consistently use `Money` object arithmetic to prevent similar errors. This was also codified in `systemPatterns.md` and `.roo/rules/02-coding-style.txt`.
