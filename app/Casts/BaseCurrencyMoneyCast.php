...
class BaseCurrencyMoneyCast extends MoneyCast
{
    ...
    protected function resolveCurrency(Model $model): Currency
    {
        ...
        if (method_exists($model, 'company') && $model->relationLoaded('company') && $model->company) {
            return $model->company->currency;
        }
        if (method_exists($model, 'journalEntry') && $model->relationLoaded('journalEntry') && $model->journalEntry) {
            return $model->journalEntry->company->currency;
        }
        if (method_exists($model, 'asset') && $model->relationLoaded('asset') && $model->asset && method_exists($model->asset, 'company')) {
