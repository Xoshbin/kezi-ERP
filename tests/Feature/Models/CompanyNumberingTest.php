...
describe('Company Numbering Settings', function () {
    ...
    it('provides default numbering settings', function () {
        ...
        expect($defaults['invoice']['type'])->toBe(\Kezi\Foundation\Enums\Settings\NumberingType::SLASH_YEAR_MONTH->value);
        expect($defaults['invoice']['prefix'])->toBe('INV');
        expect($defaults['invoice']['padding'])->toBe(7);

        expect($defaults['vendor_bill']['type'])->toBe(\Kezi\Foundation\Enums\Settings\NumberingType::SLASH_YEAR_MONTH->value);
    ...
    it('returns custom settings when set', function () {
        $customSettings = [
            'invoice' => [
                'type' => \Kezi\Foundation\Enums\Settings\NumberingType::YEAR_PREFIX->value,
                'prefix' => 'INVOICE',
                'padding' => 6,
            ],
            'vendor_bill' => [
                'type' => \Kezi\Foundation\Enums\Settings\NumberingType::SLASH_SEPARATED->value,
                'prefix' => 'PURCHASE',
                'padding' => 4,
            ],
        ];
    ...
    it('prevents numbering changes when posted invoices exist', function () {
        ...
        $invoice = \Kezi\Sales\Models\Invoice::factory()->create([
    ...
    it('prevents numbering changes when both posted invoices and bills exist', function () {
        ...
        \Kezi\Sales\Models\Invoice::factory()->create([
            'company_id' => $this->company->id,
            'status' => InvoiceStatus::Posted,
            'invoice_number' => 'INV-00001',
        ]);

        \Kezi\Purchase\Models\VendorBill::factory()->create([
            'company_id' => $this->company->id,
            'status' => VendorBillStatus::Posted,
            'bill_reference' => 'BILL-00001',
        ]);
