...
use Kezi\Accounting\Exceptions\PeriodIsLockedException;
...
function setupFoundation()
{
    ...
    User::where('email', 'soran@kezierp.com')->delete();
    $user = User::create([
        'name' => 'Soran',
    ...
    // Create accounts
    $accountsData = [
        '1010' => ['name' => 'Bank', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::BankAndCash],
        '1200' => ['name' => 'Accounts Receivable', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Receivable],
        '1500' => ['name' => 'IT Equipment', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::FixedAssets, 'can_create_assets' => true],
        '1501' => ['name' => 'Accumulated Depreciation', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::NonCurrentAssets],
        '2100' => ['name' => 'Accounts Payable', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Payable],
        '3000' => ['name' => 'Owner\'s Equity', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Equity],
        '4000' => ['name' => 'Consulting Revenue', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income],
        '5000' => ['name' => 'Sales Discounts & Returns', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income],
        '6100' => ['name' => 'Depreciation Expense', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Depreciation],
    ];

    $accounts = [];
    foreach ($accountsData as $code => $data) {
        $accounts[$code] = \Kezi\Accounting\Models\Account::create([
...
test('Kezi ERP complete accounting scenario - Full Workflow', function () {
    ...
    livewire(CreateJournalEntry::class)
        ->fillForm([
            ...
            'description' => 'Soran\'s personal funds transferred to the Kezi ERP bank account',
        ])
        ->set('data.lines', [
            [
    ...
    livewire(CreatePartner::class)
        ->fillForm([
            ...
            'type' => \Kezi\Foundation\Enums\Partners\PartnerType::Vendor->value,
        ])
        ->call('create')
        ->assertHasNoFormErrors();

    $vendor = \Kezi\Foundation\Models\Partner::where('name', 'Paykar Tech Supplies')->first();
    ...
    $vendorBill = \Kezi\Purchase\Models\VendorBill::where('bill_reference', 'KE-LAPTOP-001')->first();
    ...
    expect($vendorBillJE->source_type)->toBe(\Kezi\Purchase\Models\VendorBill::class);
    ...
    $createdAssets = Asset::where('source_type', \Kezi\Purchase\Models\VendorBill::class)
    ...
    livewire(CreatePartner::class)
        ->fillForm([
            ...
            'type' => \Kezi\Foundation\Enums\Partners\PartnerType::Customer->value,
        ])
        ->call('create')
        ->assertHasNoFormErrors();

    $customer = \Kezi\Foundation\Models\Partner::where('name', 'Hawre Trading Group')->first();
    ...
    $invoice = \Kezi\Sales\Models\Invoice::where('customer_id', $customer->id)->first();
    ...
    expect($invoiceJE->source_type)->toBe(\Kezi\Sales\Models\Invoice::class);
    ...
    $payment = \Kezi\Payment\Models\Payment::where('reference', 'Payment from Hawre Trading Group')->first();
    ...
    livewire(\Kezi\Accounting\Filament\Clusters\Accounting\Resources\VendorBills\Pages\ViewVendorBill::class, [
        'record' => $vendorBill->getRouteKey(),
    ])
        ->callAction('register_payment', [
            'journal_id' => $journals['Bank']->id,
    ...
    $vendorPayment = \Kezi\Payment\Models\Payment::where('reference', 'Payment to Paykar Tech Supplies')->first();
    ...
    $creditNote = \Kezi\Inventory\Models\AdjustmentDocument::where('original_invoice_id', $invoice->id)->first();
    ...
    expect($creditNoteJE->source_type)->toBe(\Kezi\Inventory\Models\AdjustmentDocument::class);
    ...
    $bankStatement = \Kezi\Accounting\Models\BankStatement::where('reference', 'Monthly Statement - '.now()->format('Y-m'))->first();
    ...
    $inventoryAccounts = [
        ['code' => '1100', 'name' => 'Inventory Asset', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::CurrentAssets],
        ['code' => '6000', 'name' => 'Cost of Goods Sold', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::CostOfRevenue],
        ['code' => '2150', 'name' => 'Stock Interim (Received)', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::CurrentLiabilities],
    ];
    ...
    foreach ($inventoryAccounts as $accountData) {
        ...
        $account = \Kezi\Accounting\Models\Account::where('code', $accountData['code'])->first();
    ...
    $product = \Kezi\Product\Models\Product::where('sku', 'ITWS001')->first();
    ...
    $inventoryVendor = \Kezi\Foundation\Models\Partner::where('name', 'Global Tech Distributors')->first();
    ...
    $inventoryBill = \Kezi\Purchase\Models\VendorBill::where('bill_reference', 'GBL-WS-001')->first();
    ...
    $lockDateRecord = \Kezi\Accounting\Models\LockDate::create([
        'company_id' => $company->id,
