...
beforeEach(function () {
    ...
    $this->receivableAccount = Account::factory()->create([
        'company_id' => $this->company->id,
        'type' => \Kezi\Accounting\Enums\Accounting\AccountType::CurrentAssets, // Employee Advances
        'currency_id' => $this->company->currency_id,
    ]);
    ...
    $this->cashJournal = Journal::factory()->create([
        'company_id' => $this->company->id,
        'type' => \Kezi\Accounting\Enums\Accounting\JournalType::Cash,
        'currency_id' => $this->company->currency_id,
...
test('full cash advance lifecycle', function () {
    ...
    $expenseDto = new \Kezi\HR\DataTransferObjects\HumanResources\CreateExpenseReportDTO(
        company_id: $this->company->id,
    ...
    expect($report->fresh()->status)->toBe(\Kezi\HR\Enums\ExpenseReportStatus::Submitted);
    ...
    expect($report->fresh()->status)->toBe(\Kezi\HR\Enums\ExpenseReportStatus::Approved);
