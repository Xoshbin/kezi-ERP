...
namespace Kezi\Payment\Services;
...
use Kezi\Accounting\Services\ExchangeGainLossService;
...
use Kezi\Payment\Events\PaymentConfirmed;
...
class PaymentService
{
    ...
    public function confirm(Payment $payment, User $user): Payment
    {
        if ($payment->status !== PaymentStatus::Draft) {
            throw new \Kezi\Foundation\Exceptions\UpdateNotAllowedException('Only draft payments can be confirmed.');
        }
     ...
     *
     * @throws \Kezi\Foundation\Exceptions\DeletionNotAllowedException If the payment is not in a draft state.
     */
    public function delete(Payment $payment): void
    {
        // THE GUARD CLAUSE: This is the core of the fix.
        if ($payment->status !== PaymentStatus::Draft) {
            throw new \Kezi\Foundation\Exceptions\DeletionNotAllowedException('Confirmed payments cannot be deleted. Please create a reversal entry instead.');
        }
