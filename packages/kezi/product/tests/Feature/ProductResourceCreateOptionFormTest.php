...
test('product create page loads without getRecord error', function () {
    ...
    $this->get(route('filament.kezi.inventory.resources.products.create', [
        'tenant' => $this->company,
    ]))
        ->assertSuccessful()
...
test('can create account through createOptionForm with proper company_id', function () {
    ...
    $accountData = [
        'code' => 'TEST-001',
        'name' => 'Test Account',
        'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income->value,
        'is_deprecated' => false,
    ];
    ...
    expect($account->type)->toBe(\Kezi\Accounting\Enums\Accounting\AccountType::Income);
});
