...
namespace Kezi\Accounting\Filament\Clusters\Accounting\Widgets;
...
use Kezi\Accounting\DataTransferObjects\Reports\BalanceSheetDTO;
...
class FinancialStatsOverview extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        // Get services
        $plService = app(\Kezi\Accounting\Services\Reports\ProfitAndLossStatementService::class);
        $bsService = app(\Kezi\Accounting\Services\Reports\BalanceSheetService::class);
        $arService = app(\Kezi\Accounting\Services\Reports\AgedReceivableService::class);
        $apService = app(\Kezi\Accounting\Services\Reports\AgedPayableService::class);
        ...
        return [
            ...
            Stat::make(__('accounting::dashboard.financial.current_month_profit'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($netProfit))
            ...
            Stat::make(__('accounting::dashboard.financial.ytd_profit'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($ytdNetProfit))
            ...
            Stat::make(__('accounting::dashboard.financial.total_receivables'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($totalReceivables))
            ...
            Stat::make(__('accounting::dashboard.financial.total_payables'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($totalPayables))
                ->description(__('accounting::dashboard.financial.outstanding_vendor_bills'))
                ->descriptionIcon('heroicon-m-credit-card')
                ->color($totalPayables->isZero() ? 'gray' : 'danger'),

            // Cash Balance
            Stat::make(__('accounting::dashboard.financial.cash_balance'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($cashBalance))
            ...
            // Gross Profit Margin
            Stat::make(__('accounting::dashboard.financial.gross_margin'), \Kezi\Foundation\Support\NumberFormatter::formatPercentage($grossProfitMargin, 1))
     ...
     */
    private function getMonthlyProfitTrend(Company $company, \Kezi\Accounting\Services\Reports\ProfitAndLossStatementService $plService): array
    {
        $data = [];
