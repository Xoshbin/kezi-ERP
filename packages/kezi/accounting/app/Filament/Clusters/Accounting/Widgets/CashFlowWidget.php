...
class CashFlowWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        try {
            ...
            return [
                Stat::make(__('accounting::dashboard.cash_flow.error'), __('accounting::dashboard.cash_flow.data_unavailable'))
                    ->description(__('accounting::dashboard.cash_flow.please_check_setup'))
                    ->color('danger')
        ...
        return [
            ...
            Stat::make(__('accounting::dashboard.cash_flow.overdue_receivables'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($overdueReceivables))
                ->description(__('accounting::dashboard.cash_flow.immediate_collection_needed'))
                ->descriptionIcon('heroicon-m-exclamation-triangle')
                ->color($overdueReceivables->isZero() ? 'success' : 'danger'),

            // Overdue Payables (90+ days)
            Stat::make(__('accounting::dashboard.cash_flow.overdue_payables'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($overduePayables))
                ->description(__('accounting::dashboard.cash_flow.immediate_payment_needed'))
                ->descriptionIcon('heroicon-m-exclamation-triangle')
                ->color($overduePayables->isZero() ? 'success' : 'warning'),

            // Near-term Cash Flow (Current + 1-30 days)
            Stat::make(__('accounting::dashboard.cash_flow.forecast_near_term'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($netCashFlowSoon))
                ->description(__('accounting::dashboard.cash_flow.net_cash_flow_soon'))
            ...
            Stat::make(__('accounting::dashboard.cash_flow.forecast_30_days'), \Kezi\Foundation\Support\NumberFormatter::formatMoneyTo($netCashFlow30Days))
                ->description(__('accounting::dashboard.cash_flow.net_cash_flow_month'))
