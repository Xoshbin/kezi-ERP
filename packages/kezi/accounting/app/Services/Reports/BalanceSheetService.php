...
use Kezi\Accounting\Exceptions\BalanceSheetNotBalancedException;
...
class BalanceSheetService
{
    public function generate(Company $company, Carbon $asOfDate): BalanceSheetDTO
    {
        ...
        $assetLines = $this->mapBalancesToReportLines($accountBalances, \Kezi\Accounting\Enums\Accounting\AccountType::assetTypes(), $currency, $accounts);
        $liabilityLines = $this->mapBalancesToReportLines($accountBalances, \Kezi\Accounting\Enums\Accounting\AccountType::liabilityTypes(), $currency, $accounts, true);
        $equityLines = $this->mapBalancesToReportLines($accountBalances, \Kezi\Accounting\Enums\Accounting\AccountType::equityTypes(), $currency, $accounts, true);
    ...
    private function getAccountBalances(Company $company, Carbon $asOfDate): Collection
    {
        ...
        $results = DB::table('journal_entry_lines')
            ...
            ->whereIn('accounts.type', array_map(fn ($type) => $type->value, \Kezi\Accounting\Enums\Accounting\AccountType::balanceSheetTypes()))
    ...
    private function getCurrentYearEarnings(Company $company, Carbon $fiscalYearStart, Carbon $asOfDate): Money
    {
        ...
        $results = DB::table('journal_entry_lines')
            ...
            ->whereIn('accounts.type', [
                \Kezi\Accounting\Enums\Accounting\AccountType::Income->value,
                \Kezi\Accounting\Enums\Accounting\AccountType::OtherIncome->value,
                \Kezi\Accounting\Enums\Accounting\AccountType::Expense->value,
                \Kezi\Accounting\Enums\Accounting\AccountType::Depreciation->value,
                \Kezi\Accounting\Enums\Accounting\AccountType::CostOfRevenue->value,
            ])
            ->where('journal_entries.state', JournalEntryState::Posted->value)
    ...
    /**
     * @param  array<\Kezi\Accounting\Enums\Accounting\AccountType>  $types
