...
test('it creates a journal entry in a foreign currency correctly', function () {
    ...
    $journalEntry = resolve(\Kezi\Accounting\Actions\Accounting\CreateJournalEntryAction::class)->execute($dto);
...
test('it blocks posting to a currency-locked account with the wrong currency', function () {
    ...
    expect(fn () => resolve(\Kezi\Accounting\Actions\Accounting\CreateJournalEntryAction::class)->execute($dto))
