...
use Kezi\Accounting\DataTransferObjects\Reports\CashFlowStatementDTO;
...
test('it adds back depreciation in operating activities', function () {
    ...
    $salesAccount = Account::factory()->for($this->company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income]);
    $depreciationAccount = Account::factory()->for($this->company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::Depreciation]);
    $accumulatedDepAccount = Account::factory()->for($this->company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::FixedAssets]);
