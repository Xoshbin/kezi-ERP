...
test('it generates a balanced trial balance report', function () {
    ...
    $bankAccount = Account::factory()->for($company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::BankAndCash]);
    $salesAccount = Account::factory()->for($company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income]);
    $expenseAccount = Account::factory()->for($company)->create(['type' => \Kezi\Accounting\Enums\Accounting\AccountType::Expense]);
    ...
    // Action
    $service = app(\Kezi\Accounting\Services\Reports\TrialBalanceService::class);
...
test('it orders accounts by account code', function () {
    ...
    $account1 = Account::factory()->for($company)->create(['code' => '1000', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::BankAndCash]);
    $account2 = Account::factory()->for($company)->create(['code' => '4000', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income]);
    $account3 = Account::factory()->for($company)->create(['code' => '2000', 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Expense]);
