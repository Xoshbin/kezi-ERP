...
namespace Kezi\Accounting\Tests\Feature\Filament\Clusters\Accounting\Resources\Invoices;
...
beforeEach(function () {
    ...
    $this->partnerB = Partner::factory()->create([
        ...
        'payable_account_id' => Account::factory()->create([
            'company_id' => $this->companyA->id,
            'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Payable,
        ])->id,
    ]);
});
...
it('auto-creates vendor bill in company B when invoice to partner B is confirmed in company A', function () {
    ...
    $product = Product::factory()->create([
        'company_id' => $this->companyA->id,
        'income_account_id' => Account::factory()->create(['company_id' => $this->companyA->id, 'type' => \Kezi\Accounting\Enums\Accounting\AccountType::Income])->id,
    ]);
    ...
    // Create Invoice Line
    \Kezi\Sales\Models\InvoiceLine::factory()->create([
    ...
    $partnerA = \Kezi\Foundation\Models\Partner::factory()->create([
    ...
    \Kezi\Accounting\Models\Account::factory()->create([
        'company_id' => $this->companyB->id,
    ...
    if ($invoice->fresh()->status !== \Kezi\Sales\Enums\Sales\InvoiceStatus::Posted) {
        $test->assertNotified();
    }
