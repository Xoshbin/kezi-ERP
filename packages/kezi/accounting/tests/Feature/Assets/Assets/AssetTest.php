...
test('a draft asset can be deleted', function () {
    ...
    $assetService = app(\Kezi\Accounting\Services\AssetService::class);
...
test('a confirmed asset cannot be deleted', function () {
    ...
    expect(fn () => $assetService->delete($asset))
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a confirmed asset. Only draft assets can be deleted.');
...
test('an asset with depreciation entries cannot be deleted', function () {
    ...
    $assetService = app(\Kezi\Accounting\Services\AssetService::class);
    expect(fn () => $assetService->delete($asset))
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete an asset with depreciation entries. Depreciation history must be preserved.');
...
test('an asset with journal entries cannot be deleted', function () {
    ...
    $assetService = app(\Kezi\Accounting\Services\AssetService::class);
    expect(fn () => $assetService->delete($asset))
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete an asset with associated journal entries. Financial records must be preserved.');
