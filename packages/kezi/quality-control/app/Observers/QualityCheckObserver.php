...
namespace Kezi\QualityControl\Observers;

use App\Models\User;
use Kezi\QualityControl\Actions\RejectLotAction;
use Kezi\QualityControl\DataTransferObjects\RejectLotDTO;
    ...
    public function updated(QualityCheck $qualityCheck): void
    {
        if ($qualityCheck->isDirty('status') && $qualityCheck->status === QualityCheckStatus::Failed) {
            ...
            if ($qualityCheck->is_blocking) {
                app(\Kezi\QualityControl\Actions\CreateQualityAlertAction::class)->execute(
                    new \Kezi\QualityControl\DataTransferObjects\CreateQualityAlertDTO(
