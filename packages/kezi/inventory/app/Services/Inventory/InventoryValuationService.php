...
use Kezi\Inventory\DataTransferObjects\Inventory\CostDeterminationResult;
...
class InventoryValuationService
{
     ...
     *
     * @param  \Kezi\Accounting\Services\Accounting\LockDateService  $lockDateService  Service for enforcing accounting lock dates
     * @param  StockQuantService  $stockQuantService  Service for managing stock quantities
     * @param  \Kezi\Foundation\Services\CurrencyConverterService  $currencyConverter  Service for currency conversion
     */
    public function __construct(
        ...
        protected \Kezi\Foundation\Services\CurrencyConverterService $currencyConverter,
    ) {}
     ...
     * @throws \Kezi\Accounting\Exceptions\PeriodIsLockedException When transaction date is before lock date
     *
     * @example
    ...
    public function processIncomingStock(Product $product, float $quantity, Money $costPerUnit, Carbon $date, Model $sourceDocument): void
    {
        ...
        if ($sourceDocument instanceof \Kezi\Manufacturing\Models\ManufacturingOrder) {
     ...
     *
     * @throws \Kezi\Accounting\Exceptions\PeriodIsLockedException When adjustment date is before lock date
    ...
    private function createCOGSJournalEntry(Product $product, Money $cogsAmount, Carbon $date, Model $sourceDocument): JournalEntry
    {
        ...
        return app(\Kezi\Accounting\Actions\Accounting\CreateJournalEntryAction::class)->execute($journalEntryDTO);
    }
    ...
    public function createConsolidatedManualStockMoveJournalEntry(StockMove $stockMove): ?JournalEntry
    {
        ...
        $journalEntry = app(\Kezi\Accounting\Actions\Accounting\CreateJournalEntryAction::class)->execute($journalEntryDTO);
    ...
    public function calculateIncomingCostPerUnitEnhanced(
        ...
        if ($stockMove->source_type === 'Kezi\Purchase\Models\VendorBill') {
