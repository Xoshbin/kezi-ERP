...
it('auto-confirms delivery when company inventory_accounting_mode is automatic', function () {
    ...
    $this->company->update([
        'inventory_accounting_mode' => \Kezi\Inventory\Enums\Inventory\InventoryAccountingMode::AUTO_RECORD_ON_BILL,
    ]);
    ...
    $salesOrder = \Kezi\Sales\Models\SalesOrder::factory()->for($this->company)->create([
        'customer_id' => $this->customer->id,
    ...
    \Kezi\Sales\Models\SalesOrderLine::factory()->create([
    ...
    $confirmAction = app(\Kezi\Sales\Actions\Sales\ConfirmSalesOrderAction::class);
    $confirmAction->execute($salesOrder, $this->user);

    // Assert: Stock picking was created and is in 'done' state
    $stockPicking = \Kezi\Inventory\Models\StockPicking::where('origin', 'LIKE', '%'.$salesOrder->so_number.'%')->first();
    expect($stockPicking)->not->toBeNull();
    expect($stockPicking->state)->toBe(\Kezi\Inventory\Enums\Inventory\StockPickingState::Done);

    // Assert: Stock move was created and is in 'done' status
    $stockMove = StockMove::where('source_type', \Kezi\Sales\Models\SalesOrder::class)
...
it('does not auto-confirm delivery when company inventory_accounting_mode is manual', function () {
    ...
    $this->company->update([
        'inventory_accounting_mode' => \Kezi\Inventory\Enums\Inventory\InventoryAccountingMode::MANUAL_INVENTORY_RECORDING,
    ]);
    ...
    $stockPicking = \Kezi\Inventory\Models\StockPicking::where('origin', 'LIKE', '%'.$salesOrder->so_number.'%')->first();
    expect($stockPicking)->not->toBeNull();
    expect($stockPicking->state)->toBe(\Kezi\Inventory\Enums\Inventory\StockPickingState::Draft);
