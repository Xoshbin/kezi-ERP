...
it('creates non-zero inventory journal amounts for manual incoming stock moves', function () {
    ...
    // Act
    $move = app(\Kezi\Inventory\Actions\Inventory\CreateStockMoveAction::class)->execute($dto);
    $move->refresh();
...
it('throws exception for outgoing stock moves when product has no cost information via event-driven COGS', function () {
    ...
    expect(fn () => app(\Kezi\Inventory\Services\Inventory\StockMoveService::class)->confirmMove(
        new \Kezi\Inventory\DataTransferObjects\Inventory\ConfirmStockMoveDTO(stock_move_id: $move->id)
