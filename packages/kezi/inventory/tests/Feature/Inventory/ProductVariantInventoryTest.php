...
it('lot tracking works per variant', function () {
    ...
    $this->template->update(['tracking_type' => \Kezi\Inventory\Enums\Inventory\TrackingType::Lot]);

    $variant = $this->variants->first();
    // Simulate inheritance (since sync logic is deferred to Phase 2)
    $variant->update(['tracking_type' => \Kezi\Inventory\Enums\Inventory\TrackingType::Lot]);

    // Verify variant inherits lot tracking
    expect($variant->tracking_type)->toBe(\Kezi\Inventory\Enums\Inventory\TrackingType::Lot);

    // Create a lot for the variant
    $lot = \Kezi\Inventory\Models\Lot::create([
        'company_id' => $this->company->id,
        'product_id' => $variant->id,
        'lot_code' => 'LOT-001',
    ]);
    ...
    $journal = \Kezi\Accounting\Models\Journal::factory()->create([
        'company_id' => $this->company->id,
    ...
    $po = \Kezi\Purchase\Models\PurchaseOrder::factory()->create([
        'company_id' => $this->company->id,
        'status' => \Kezi\Purchase\Enums\Purchases\PurchaseOrderStatus::Confirmed,
        'currency_id' => $this->company->currency_id,
    ]);
    $poLine = \Kezi\Purchase\Models\PurchaseOrderLine::factory()->create([
    ...
    $productLineDto = new \Kezi\Inventory\DataTransferObjects\Inventory\CreateStockMoveProductLineDTO(
        product_id: $variant->id,
        quantity: 100.0,
        from_location_id: $this->fromLocation->id,
        to_location_id: $this->toLocation->id,
    ...
    $dto = new \Kezi\Inventory\DataTransferObjects\Inventory\CreateStockMoveDTO(
        company_id: $this->company->id,
        product_lines: [$productLineDto],
        move_type: \Kezi\Inventory\Enums\Inventory\StockMoveType::Incoming,
        status: \Kezi\Inventory\Enums\Inventory\StockMoveStatus::Draft,
        move_date: now(),
        reference: 'SM-LOT-001',
        description: 'Lot tracking test move',
        source_type: \Kezi\Purchase\Models\PurchaseOrderLine::class,
        source_id: $poLine->id,
        created_by_user_id: $this->user->id
    );
    ...
    // Confirm the move
    $confirmDto = new \Kezi\Inventory\DataTransferObjects\Inventory\ConfirmStockMoveDTO(
        stock_move_id: $stockMove->id,
    );
    $this->stockMoveService->confirmMove($confirmDto);

    // Verify stock quant has lot
    $stockQuant = \Kezi\Inventory\Models\StockQuant::where('product_id', $variant->id)
        ->where('location_id', $this->toLocation->id)
        ->where('lot_id', $lot->id)
        ->first();
