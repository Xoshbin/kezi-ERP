...
namespace Kezi\Foundation\Tests\Unit\Enums;
...
class IncotermTest extends TestCase
{
    ...
    public function test_should_buyer_pay_for()
    {
        ...
        $this->assertFalse(Incoterm::Ddp->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Freight));
        $this->assertFalse(Incoterm::Ddp->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Insurance));

        // EXW: Buyer pays everything
        $this->assertTrue(Incoterm::Exw->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Freight));
        $this->assertTrue(Incoterm::Exw->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Insurance));

        // FOB: Buyer pays main carriage freight
        $this->assertTrue(Incoterm::Fob->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Freight));
        // FOB: Buyer usually pays insurance too (seller's responsibility ends at ship's rail)
        $this->assertTrue(Incoterm::Fob->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Insurance));

        // CIF: Seller pays freight and insurance
        $this->assertFalse(Incoterm::Cif->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Freight));
        $this->assertFalse(Incoterm::Cif->shouldBuyerPayFor(\Kezi\Foundation\Enums\ShippingCostType::Insurance));
    }

    public function test_get_cost_responsibilities_returns_dto()
    {
        $dto = Incoterm::Cif->getCostResponsibilities();
        $this->assertInstanceOf(\Kezi\Foundation\DataTransferObjects\ShippingCostResponsibilityDTO::class, $dto);
