...
namespace Kezi\Foundation\Tests\Feature\General;
...
test('a product linked to a posted vendor bill line cannot be deleted', function () {
    ...
    expect(fn () => $product->delete())
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a product that has been used in transactions.');
...
test('a user who created a journal entry cannot be deleted', function () {
    ...
    $this->expectException(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class);
...
test('a posted invoice with lines cannot be deleted', function () {
    ...
    $this->mock(\Kezi\Accounting\Actions\Accounting\CreateJournalEntryForInvoiceAction::class, function ($mock) {
        $mock->shouldReceive('execute')->once()->andReturn(
            \Kezi\Accounting\Models\JournalEntry::factory()->create([
