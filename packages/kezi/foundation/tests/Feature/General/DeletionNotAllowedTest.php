...
namespace Kezi\Foundation\Tests\Feature;
...
test('a company with any financial records cannot be deleted', function (string $relatedModel, array $factoryState = []) {
    ...
    expect(fn () => $this->company->delete())
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a company with associated financial records.');
...
test('a journal with journal entries cannot be deleted', function () {
    ...
    expect(fn () => $journal->delete())
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a journal with associated journal entries.');
...
test('a currency in use by a company or transaction cannot be deleted', function () {
    ...
    expect(fn () => $currencyInUse->delete())
        ->toThrow(\Kezi\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a currency that is in use.');
