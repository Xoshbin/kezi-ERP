...
class TranslatableSelectSearchTest extends TestCase
{
    ...
    public function test_product_resource_income_account_filtering()
    {
        ...
        $incomeResults = $searchService->getFilamentSearchResults(Account::class, '', [
            ...
            'searchLocales' => $searchLocales,
            'queryModifier' => fn ($query) => $query->where('company_id', $this->company->id)
                ->whereIn('type', [\Kezi\Accounting\Enums\Accounting\AccountType::Income, \Kezi\Accounting\Enums\Accounting\AccountType::OtherIncome]),
            'limit' => 50,
        ]);
    ...
    public function test_product_resource_expense_account_filtering()
    {
        ...
        $expenseResults = $searchService->getFilamentSearchResults(Account::class, '', [
            ...
            'searchLocales' => $searchLocales,
            'queryModifier' => fn ($query) => $query->where('company_id', $this->company->id)
                ->whereIn('type', [\Kezi\Accounting\Enums\Accounting\AccountType::Expense, \Kezi\Accounting\Enums\Accounting\AccountType::Depreciation, \Kezi\Accounting\Enums\Accounting\AccountType::CostOfRevenue]),
            'limit' => 50,
        ]);
