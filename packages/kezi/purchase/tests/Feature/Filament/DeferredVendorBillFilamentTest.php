...
it('can create a vendor bill with deferred expense dates on line items', function () {
    ...
    \Kezi\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'purchase']);
...
it('creates deferred item when confirming vendor bill via UI', function () {
    ...
    $this->company->update([
        'default_purchase_journal_id' => \Kezi\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'purchase'])->id,
    ...
    $deferredItem = DeferredItem::where('source_type', \Kezi\Purchase\Models\VendorBillLine::class)
