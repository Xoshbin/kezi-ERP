...
use Kezi\Foundation\Services\ExchangeRateService;
...
class VendorBillService
{
    public function __construct(
        ...
        protected \Kezi\Purchase\Services\ShippingCostAllocationService $shippingCostAllocationService,
    ...
    public function cancel(VendorBill $vendorBill, User $user, string $reason): void
    {
        ...
        DB::transaction(function () use ($vendorBill, $user, $reason) {
            ...
            $assets = \Kezi\Accounting\Models\Asset::where('source_type', get_class($vendorBill))
                ->where('source_id', $vendorBill->id)
                ->get();

            foreach ($assets as $asset) {
                if ($asset->status === \Kezi\Accounting\Enums\Assets\AssetStatus::Draft) {
                    $asset->delete();
                }
            }
        });
     ...
     */
    public function validateShippingCosts(VendorBill $vendorBill): \Kezi\Purchase\DataTransferObjects\Purchases\ShippingCostValidationResult
    {
