...
namespace Kezi\Sales\Tests\Feature\Actions;
...
it('marks sales order as fully delivered when all items delivered', function () {
    ...
    $picking = \Kezi\Inventory\Models\StockPicking::where('origin', 'Sales Order: '.$salesOrder->so_number)->first();
    expect($picking)->not->toBeNull();
    expect($picking->state)->toBe(\Kezi\Inventory\Enums\Inventory\StockPickingState::Done);
});
