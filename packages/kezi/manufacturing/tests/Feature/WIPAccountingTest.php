...
beforeEach(function () {
    ...
    $this->bom = BillOfMaterial::create([
        ...
        'quantity' => 1,
        'type' => \Kezi\Manufacturing\Enums\BOMType::Normal,
    ]);
...
test('consumption creates wip entries', function () {
    ...
    $journalEntry = \Kezi\Accounting\Models\JournalEntry::where('source_type', ManufacturingOrder::class)
...
test('production clears wip entries', function () {
    ...
    // 1. Consumption Entry
    $consumptionEntry = \Kezi\Accounting\Models\JournalEntry::where('source_type', ManufacturingOrder::class)
        ->where('source_id', $mo->id)
        ->where('description', 'like', '%Component Consumption%')
        ->first();
    ...
    $productionEntry = \Kezi\Accounting\Models\JournalEntry::find($mo->journal_entry_id);
