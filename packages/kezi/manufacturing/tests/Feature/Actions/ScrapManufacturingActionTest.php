...
namespace Kezi\Manufacturing\tests\Feature\Actions;
...
use Kezi\Manufacturing\Actions\ScrapManufacturingAction;
...
class ScrapManufacturingActionTest extends TestCase
{
    ...
    public function test_it_can_scrap_components_and_generate_accounting_entries()
    {
        ...
        $journalEntry = \Kezi\Accounting\Models\JournalEntry::where('source_type', StockMove::class)
            ->where('source_id', $scrapMove->id)
            ->first();

        // Check StockMoveValuation
        $valuation = \Kezi\Inventory\Models\StockMoveValuation::where('stock_move_id', $scrapMove->id)->first();
