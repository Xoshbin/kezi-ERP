...
describe('ProduceFinishedGoodsAction', function () {
    it('completes production and verifies exact cost calculation for Finished Goods', function () {
        ...
        // Create work center
        $workCenter = \Jmeryar\Manufacturing\Models\WorkCenter::factory()->create(['company_id' => $this->company->id]);

        // Create a work order to be completed by the action
        \Jmeryar\Manufacturing\Models\WorkOrder::create([
            'company_id' => $this->company->id,
            'manufacturing_order_id' => $mo->id,
            'work_center_id' => $workCenter->id,
            'sequence' => 1,
            'name' => 'Test Work Order',
            'status' => \Jmeryar\Manufacturing\Enums\WorkOrderStatus::Ready,
        ]);
        ...
        $mockStockMoveService->shouldReceive('createMove')
            ...
            ->andReturn(new \Jmeryar\Inventory\Models\StockMove);
