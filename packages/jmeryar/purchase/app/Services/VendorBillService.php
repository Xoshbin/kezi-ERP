...
use Jmeryar\Accounting\Actions\Accounting\BuildVendorBillPostingPreviewAction;
use Jmeryar\Accounting\Contracts\VendorBillJournalEntryCreatorContract;
...
class VendorBillService
{
    public function __construct(
        ...
        protected \Jmeryar\Purchase\Services\ShippingCostAllocationService $shippingCostAllocationService,
        protected \Jmeryar\Accounting\Services\BudgetControlService $budgetControlService,
    ) {}
    ...
    public function cancel(VendorBill $vendorBill, User $user, string $reason): void
    {
        ...
        DB::transaction(function () use ($vendorBill, $user, $reason) {
            ...
            $assets = \Jmeryar\Accounting\Models\Asset::where('source_type', get_class($vendorBill))
                ->where('source_id', $vendorBill->id)
                ->get();

            foreach ($assets as $asset) {
                if ($asset->status === \Jmeryar\Accounting\Enums\Assets\AssetStatus::Draft) {
                    $asset->delete();
                }
            }
        });
     ...
     */
    public function validateShippingCosts(VendorBill $vendorBill): \Jmeryar\Purchase\DataTransferObjects\Purchases\ShippingCostValidationResult
    {
