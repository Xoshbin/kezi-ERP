...
class EditRequestForQuotation extends EditRecord
{
    ...
    {
        /** @var \Jmeryar\Purchase\Models\RequestForQuotation $record */
        $lines = collect($data['lines'] ?? [])->map(function ($lineData) use ($record) {
            ...
            return new \Jmeryar\Purchase\DataTransferObjects\Purchases\CreateRFQLineDTO(
                description: $lineData['description'],
                quantity: (float) $lineData['quantity'],
                product: isset($lineData['product_id']) ? \Jmeryar\Product\Models\Product::find($lineData['product_id']) : null,
                tax: isset($lineData['tax_id']) ? \Jmeryar\Accounting\Models\Tax::find($lineData['tax_id']) : null,
        ...
        return app(\Jmeryar\Purchase\Actions\Purchases\UpdateRequestForQuotationAction::class)->execute($dto);
    }
}
