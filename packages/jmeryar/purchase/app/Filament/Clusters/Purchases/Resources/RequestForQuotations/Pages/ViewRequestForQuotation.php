    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('send')
                ...
                ->visible(fn (RequestForQuotationResource $resource, $record) => $record->status === \Jmeryar\Purchase\Enums\Purchases\RequestForQuotationStatus::Draft)
                ->action(fn ($record, \Jmeryar\Purchase\Services\RequestForQuotationService $service) => $service->sendRFQ($record)),

            Actions\Action::make('record_bid')
                ->label(__('purchase::request_for_quotation.actions.record_bid'))
                ->icon('heroicon-o-clipboard-document-check')
                ->visible(fn ($record) => in_array($record->status, [\Jmeryar\Purchase\Enums\Purchases\RequestForQuotationStatus::Sent, \Jmeryar\Purchase\Enums\Purchases\RequestForQuotationStatus::BidReceived]))
                ->form([
                ...
                ->action(function ($record, array $data, \Jmeryar\Purchase\Services\RequestForQuotationService $service) {
                    $dto = new \Jmeryar\Purchase\DataTransferObjects\Purchases\UpdateRFQDTO(
                        rfqId: $record->id,
                        notes: $data['notes']
                    );
            ...
            Actions\Action::make('convert_to_po')
                ...
                ->requiresConfirmation()
                ->visible(fn ($record) => in_array($record->status, [\Jmeryar\Purchase\Enums\Purchases\RequestForQuotationStatus::BidReceived, \Jmeryar\Purchase\Enums\Purchases\RequestForQuotationStatus::Accepted]) && ! $record->converted_to_purchase_order_id)
                ->form([
