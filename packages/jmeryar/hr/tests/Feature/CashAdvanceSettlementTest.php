...
beforeEach(function () {
    ...
    // Journals
    $this->generalJournal = \Jmeryar\Accounting\Models\Journal::factory()->create([
        'company_id' => $this->company->id,
...
test('settles multiple expense reports at once', function () {
    ...
    // Report 1: 300
    \Jmeryar\HR\Models\ExpenseReport::factory()->create([
        'company_id' => $this->company->id,
...
test('settles full return with zero expenses', function () {
    ...
    $returnJe = \Jmeryar\Accounting\Models\JournalEntry::where('reference', "Return {$advance->advance_number}")->first();
...
test('carry forward settlement updates status without reimbursement entry', function () {
    ...
    $reimburseJe = \Jmeryar\Accounting\Models\JournalEntry::where('reference', "Reimbursement {$advance->advance_number}")->first();
