...
beforeEach(function () {
    ...
    $this->advance = CashAdvance::factory()->create([
        ...
        'currency_id' => $this->company->currency_id,
        'status' => \Jmeryar\HR\Enums\CashAdvanceStatus::Disbursed,
    ]);
});
...
test('cannot create expense line with different currency than advance', function () {
    $otherCurrency = \Jmeryar\Foundation\Models\Currency::firstWhere('code', 'USD')
        ?? \Jmeryar\Foundation\Models\Currency::factory()->create(['code' => 'USD']);
...
test('submitting expense report updates status', function () {
    $report = \Jmeryar\HR\Models\ExpenseReport::factory()->create([
        'company_id' => $this->company->id,
        'cash_advance_id' => $this->advance->id,
        'employee_id' => $this->employee->id,
