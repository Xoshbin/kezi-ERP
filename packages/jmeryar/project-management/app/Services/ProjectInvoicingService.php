...
class ProjectInvoicingService
{
    ...
    public function createCustomerInvoice(ProjectInvoice $projectInvoice): \Jmeryar\Sales\Models\Invoice
    {
        ...
        $incomeAccount = \Jmeryar\Accounting\Models\Account::where('company_id', $projectInvoice->company_id)
            ->where('type', \Jmeryar\Accounting\Enums\Accounting\AccountType::Income)
            ->first();
        ...
        if (! $incomeAccount) {
            $incomeAccount = \Jmeryar\Accounting\Models\Account::factory()->create([
        ...
        if ($projectInvoice->labor_amount->isPositive()) {
            $lines[] = new \Jmeryar\Sales\DataTransferObjects\Sales\CreateInvoiceLineDTO(
        ...
        $invoiceDto = new \Jmeryar\Sales\DataTransferObjects\Sales\CreateInvoiceDTO(
            company_id: $projectInvoice->company_id,
