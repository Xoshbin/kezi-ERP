...
//     $receivableAccount = \Jmeryar\Sales\Models\Account::factory()->create([
//         'company_id' => $this->company->id,
//         'type' => 'receivable',
//         'name' => 'Accounts Receivable',
//     ]);

//     $salesAccount = \Jmeryar\Sales\Models\Account::factory()->create([
//         'company_id' => $this->company->id,
//         'type' => 'income',
//         'name' => 'Sales Revenue',
//     ]);
...
//     $journalEntry = \Jmeryar\Sales\Models\JournalEntry::factory()->create([
//         'company_id' => $this->company->id,
//         'is_posted' => true,
//     ]);
...
describe('Invoice Confirmation Business Rules', function () {
    ...
    it('prevents confirming invoice without line items via backend service', function () {
        ...
        })->toThrow(RuntimeException::class, 'Cannot confirm invoice without line items');
    ...
    it('prevents confirmation when invoice has zero total amount', function () {
        ...
        expect(function () use ($service, $invoice) {
            $service->confirm($invoice, $this->user);
        })->toThrow(RuntimeException::class, 'Cannot confirm invoice with zero total amount');
    });
...
it('can register payment for a posted invoice', function () {
    ...
    $bankJournal = \Jmeryar\Accounting\Models\Journal::where('company_id', $this->company->id)
        ->where('type', 'bank')
        ->first();
