...
test('a posted invoice cannot be deleted', function () {
    ...
    expect(fn () => (app(InvoiceService::class))->delete($invoice))
        ->toThrow(\Jmeryar\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a posted invoice.');
...
test('an invoice cannot be created or posted in a locked period', function () {
    ...
    */
    ...
    expect(fn () => (app(\Jmeryar\Sales\Actions\Sales\CreateInvoiceAction::class))->execute($invoiceDto))
