...
namespace Jmeryar\Payment\Tests\Feature\Actions;
...
it('creates a draft payment with withholding tax for a vendor bill', function () {
    ...
    $dto = new CreatePaymentDTO(
        ...
        payment_method: \Jmeryar\Payment\Enums\Payments\PaymentMethod::BankTransfer,
...
it('throws exception if payment is linked to both invoices and vendor bills', function () {
    // Arrange
    $invoice = \Jmeryar\Sales\Models\Invoice::factory()->for($this->company)->create();
    $vendorBill = \Jmeryar\Purchase\Models\VendorBill::factory()->for($this->company)->create();
    ...
    $dto = new CreatePaymentDTO(
        ...
        payment_method: \Jmeryar\Payment\Enums\Payments\PaymentMethod::Cash,
        paid_to_from_partner_id: null,
        amount: null,
...
it('correctly converts WHT amount to base currency in multi-currency payment', function () {
    // Arrange
    $usdCurrency = \Jmeryar\Foundation\Models\Currency::factory()->create([
        'code' => 'USD',
        'decimal_places' => 2,
    ]);

    // Set exchange rate: 1 USD = 1500 IQD (Base)
    \Jmeryar\Foundation\Models\CurrencyRate::factory()->for($this->company)->create([
