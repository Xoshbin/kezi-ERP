...
namespace Jmeryar\Payment\Actions\Payments;
...
use Jmeryar\Payment\Services\Payments\Strategies\SettlementStrategy;
...
class UpdatePaymentAction
{
    public function __construct(
        ...
        private readonly \Jmeryar\Foundation\Services\CurrencyConverterService $currencyConverter
    ) {}
    ...
    public function execute(UpdatePaymentDTO $dto): Payment
    {
        ...
        if ($payment->status !== PaymentStatus::Draft) {
            throw new \Jmeryar\Foundation\Exceptions\UpdateNotAllowedException('Only draft payments can be updated.');
        }
        ...
        return DB::transaction(function () use ($dto, $payment) {
            ...
            \Jmeryar\Accounting\Models\WithholdingTaxEntry::where('payment_id', $payment->id)->delete();
            foreach ($whtEntriesToCreate as $whtData) {
                $whtData['payment_id'] = $payment->id;
                \Jmeryar\Accounting\Models\WithholdingTaxEntry::create($whtData);
            }
