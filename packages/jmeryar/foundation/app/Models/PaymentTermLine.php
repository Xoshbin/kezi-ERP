...
namespace Jmeryar\Foundation\Models;
 ...
 * @property \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType $type
 * @property int $days
 ...
 *
 * @method static \Jmeryar\Foundation\Database\Factories\PaymentTermLineFactory factory($count = null, $state = [])
 ...
 * @mixin Eloquent
 */
#[ObservedBy([\Jmeryar\Foundation\Observers\AuditLogObserver::class])]
class PaymentTermLine extends Model
{
    use HasFactory;

    protected static function newFactory(): \Jmeryar\Foundation\Database\Factories\PaymentTermLineFactory
    {
        return \Jmeryar\Foundation\Database\Factories\PaymentTermLineFactory::new();
    }
     ...
     */
    protected $casts = [
        'type' => \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::class,
        'percentage' => 'float',
    ...
    public function calculateDueDate(Carbon $documentDate): Carbon
    {
        return match ($this->type) {
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::Net => $documentDate->copy()->addDays($this->days),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::EndOfMonth => $this->calculateEndOfMonthDate($documentDate),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::DayOfMonth => $this->calculateDayOfMonthDate($documentDate),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::Immediate => $documentDate->copy(),
        };
    }
    ...
    public function getDescription(): string
    {
        $description = match ($this->type) {
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::Immediate => __('foundation::payment_terms.immediate'),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::Net => __('foundation::payment_terms.net_days', ['days' => $this->days]),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::EndOfMonth => $this->days > 0
                ? __('foundation::payment_terms.end_of_month_plus_days', ['days' => $this->days])
                : __('foundation::payment_terms.end_of_month'),
            \Jmeryar\Foundation\Enums\PaymentTerms\PaymentTermType::DayOfMonth => __('foundation::payment_terms.day_of_month', [
