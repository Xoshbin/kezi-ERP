...
namespace Jmeryar\Foundation\Tests\Feature\General;
...
test('a partner linked to a posted invoice cannot be deleted', function () {
    ...
    expect(fn () => $customer->delete())
        ->toThrow(
            \Jmeryar\Foundation\Exceptions\DeletionNotAllowedException::class,
...
test('a product linked to a posted vendor bill line cannot be deleted', function () {
    ...
    expect(fn () => $product->delete())
        ->toThrow(\Jmeryar\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a product that has been used in transactions.');
...
test('a user who created a journal entry cannot be deleted', function () {
    ...
    $this->expectException(\Jmeryar\Foundation\Exceptions\DeletionNotAllowedException::class);
...
test('a posted invoice with lines cannot be deleted', function () {
    ...
    $this->mock(\Jmeryar\Accounting\Actions\Accounting\CreateJournalEntryForInvoiceAction::class, function ($mock) {
        $mock->shouldReceive('execute')->once()->andReturn(
            \Jmeryar\Accounting\Models\JournalEntry::factory()->create([
