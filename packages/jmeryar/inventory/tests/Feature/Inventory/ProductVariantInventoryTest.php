...
it('lot tracking works per variant', function () {
    ...
    $this->template->update(['tracking_type' => \Jmeryar\Inventory\Enums\Inventory\TrackingType::Lot]);

    $variant = $this->variants->first();
    // Simulate inheritance (since sync logic is deferred to Phase 2)
    $variant->update(['tracking_type' => \Jmeryar\Inventory\Enums\Inventory\TrackingType::Lot]);

    // Verify variant inherits lot tracking
    expect($variant->tracking_type)->toBe(\Jmeryar\Inventory\Enums\Inventory\TrackingType::Lot);

    // Create a lot for the variant
    $lot = \Jmeryar\Inventory\Models\Lot::create([
        'company_id' => $this->company->id,
        'product_id' => $variant->id,
        'lot_code' => 'LOT-001',
    ]);
    ...
    $po = \Jmeryar\Purchase\Models\PurchaseOrder::factory()->create([
        'company_id' => $this->company->id,
        'status' => \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::Confirmed,
        'currency_id' => $this->company->currency_id,
    ]);
    $poLine = \Jmeryar\Purchase\Models\PurchaseOrderLine::factory()->create([
    ...
    $productLineDto = new \Jmeryar\Inventory\DataTransferObjects\Inventory\CreateStockMoveProductLineDTO(
        product_id: $variant->id,
        quantity: 100.0,
        from_location_id: $this->fromLocation->id,
        to_location_id: $this->toLocation->id,
    ...
    $dto = new \Jmeryar\Inventory\DataTransferObjects\Inventory\CreateStockMoveDTO(
        ...
        status: \Jmeryar\Inventory\Enums\Inventory\StockMoveStatus::Draft,
        move_date: now(),
        reference: 'SM-LOT-001',
        description: 'Lot tracking test move',
        source_type: \Jmeryar\Purchase\Models\PurchaseOrderLine::class,
        source_id: $poLine->id,
        created_by_user_id: $this->user->id
    );
    ...
    // Confirm the move
    $confirmDto = new \Jmeryar\Inventory\DataTransferObjects\Inventory\ConfirmStockMoveDTO(
        stock_move_id: $stockMove->id,
    );
    $this->stockMoveService->confirmMove($confirmDto);

    // Verify stock quant has lot
    $stockQuant = \Jmeryar\Inventory\Models\StockQuant::where('product_id', $variant->id)
        ->where('location_id', $this->toLocation->id)
        ->where('lot_id', $lot->id)
        ->first();
