...
it('auto-confirms delivery when company inventory_accounting_mode is automatic', function () {
    ...
    $this->company->update([
        'inventory_accounting_mode' => \Jmeryar\Inventory\Enums\Inventory\InventoryAccountingMode::AUTO_RECORD_ON_BILL,
    ]);
    ...
    $salesOrder = \Jmeryar\Sales\Models\SalesOrder::factory()->for($this->company)->create([
        'customer_id' => $this->customer->id,
        'status' => \Jmeryar\Sales\Enums\Sales\SalesOrderStatus::Draft,
        'currency_id' => $this->company->currency_id,
        'created_by_user_id' => $this->user->id,
    ]);

    // Add sales order line
    \Jmeryar\Sales\Models\SalesOrderLine::factory()->create([
    ...
    $confirmAction = app(\Jmeryar\Sales\Actions\Sales\ConfirmSalesOrderAction::class);
    $confirmAction->execute($salesOrder, $this->user);

    // Assert: Stock picking was created and is in 'done' state
    $stockPicking = \Jmeryar\Inventory\Models\StockPicking::where('origin', 'LIKE', '%'.$salesOrder->so_number.'%')->first();
    expect($stockPicking)->not->toBeNull();
    expect($stockPicking->state)->toBe(\Jmeryar\Inventory\Enums\Inventory\StockPickingState::Done);

    // Assert: Stock move was created and is in 'done' status
    $stockMove = StockMove::where('source_type', \Jmeryar\Sales\Models\SalesOrder::class)
...
it('does not auto-confirm delivery when company inventory_accounting_mode is manual', function () {
    ...
    $this->company->update([
        'inventory_accounting_mode' => \Jmeryar\Inventory\Enums\Inventory\InventoryAccountingMode::MANUAL_INVENTORY_RECORDING,
    ]);
    ...
    $stockPicking = \Jmeryar\Inventory\Models\StockPicking::where('origin', 'LIKE', '%'.$salesOrder->so_number.'%')->first();
    expect($stockPicking)->not->toBeNull();
    expect($stockPicking->state)->toBe(\Jmeryar\Inventory\Enums\Inventory\StockPickingState::Draft);
