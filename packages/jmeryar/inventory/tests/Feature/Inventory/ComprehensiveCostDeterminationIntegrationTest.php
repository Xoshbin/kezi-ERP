...
use Jmeryar\Inventory\Actions\Inventory\CreateJournalEntryForStockMoveAction;
...
use Jmeryar\Inventory\Services\Inventory\CostValidationService;
...
it('demonstrates complete cost determination workflow from vendor bill to manual stock move', function () {
    ...
    $manualStockMove = app(\Jmeryar\Inventory\Actions\Inventory\CreateStockMoveAction::class)->execute($dto);
...
it('demonstrates complete failure scenario with actionable suggestions', function () {
    ...
    expect(fn () => app(\Jmeryar\Inventory\Actions\Inventory\CreateStockMoveAction::class)->execute($dto))
        ->toThrow(InsufficientCostInformationException::class);
});
