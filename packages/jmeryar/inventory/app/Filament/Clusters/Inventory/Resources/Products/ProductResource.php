...
namespace Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products;
...
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\Pages\EditProduct;
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\Pages\ListProducts;
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\RelationManagers\InventoryCostLayersRelationManager;
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\RelationManagers\ReorderingRulesRelationManager;
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\RelationManagers\StockMovesRelationManager;
use Jmeryar\Inventory\Filament\Clusters\Inventory\Resources\Products\RelationManagers\VariantsRelationManager;
                ...
                ->schema([
                    ...
                    Grid::make(2)->schema([
                        ...
                        Select::make('type')
                            ...
                            ->required()
                            ->options(
                                collect(\Jmeryar\Product\Enums\Products\ProductType::cases())
                                    ->mapWithKeys(fn (\Jmeryar\Product\Enums\Products\ProductType $type) => [$type->value => $type->label()])
                            )
                            ->searchable(),
                    ]),
                ...
                ->schema([
                    Grid::make(2)->schema([
                        TranslatableSelect::make('income_account_id')
                            ...
                            ->modifyQueryUsing(fn ($query) => $query->whereIn('type', [\Jmeryar\Accounting\Enums\Accounting\AccountType::Income, \Jmeryar\Accounting\Enums\Accounting\AccountType::OtherIncome]))
                            ->createOptionForm([
                                TextInput::make('code')
                                ...
                                Select::make('type')
                                    ...
                                    ->required()
                                    ->options(
                                        collect(\Jmeryar\Accounting\Enums\Accounting\AccountType::cases())
                                            ->mapWithKeys(fn (\Jmeryar\Accounting\Enums\Accounting\AccountType $type) => [$type->value => $type->label()])
                                    )
                                    ->searchable(),
                        ...
                        TranslatableSelect::make('expense_account_id')
                            ...
                            ->preload()
                            ->searchableFields(['name', 'code'])
                            ->modifyQueryUsing(fn ($query) => $query->whereIn('type', [\Jmeryar\Accounting\Enums\Accounting\AccountType::Expense, \Jmeryar\Accounting\Enums\Accounting\AccountType::Depreciation, \Jmeryar\Accounting\Enums\Accounting\AccountType::CostOfRevenue]))
                            ->createOptionForm([
                                TextInput::make('code')
                                    ->label(__('account.code'))
                                    ->required()
                    ...
                    Grid::make(2)->schema([
                        TranslatableSelect::make('purchaseTaxes')
                            ...
                            ->searchable()
                            ->preload()
                            ->modifyQueryUsing(fn ($query) => $query->whereIn('type', [\Jmeryar\Accounting\Enums\Accounting\TaxType::Purchase, \Jmeryar\Accounting\Enums\Accounting\TaxType::Both])),
                    ]),
                ]),
                ...
                ->schema([
                    Grid::make(2)->schema([
                        Select::make('inventory_valuation_method')
                            ...
                            ->searchable()
                            ->live()
                            ->visible(fn (Get $get) => $get('type') === \Jmeryar\Product\Enums\Products\ProductType::Storable->value)
                    ...
                    Grid::make(2)->schema([
                        TranslatableSelect::make('default_inventory_account_id')
                            ...
                            ->required(fn (Get $get) => $get('type') === \Jmeryar\Product\Enums\Products\ProductType::Storable->value)
                            ->rules(['required_if:type,'.\Jmeryar\Product\Enums\Products\ProductType::Storable->value])
                            ->visible(fn (Get $get) => $get('type') === \Jmeryar\Product\Enums\Products\ProductType::Storable->value)
                            ->createOptionForm([
                    ...
                    Grid::make(1)->schema([
                        Select::make('tracking_type')
                            ...
                            ->options([
                                \Jmeryar\Inventory\Enums\Inventory\TrackingType::None->value => __('inventory.tracking_type_none'),
                                \Jmeryar\Inventory\Enums\Inventory\TrackingType::Lot->value => __('inventory.tracking_type_lot'),
                                \Jmeryar\Inventory\Enums\Inventory\TrackingType::Serial->value => __('inventory.tracking_type_serial'),
                            ])
                            ->default(\Jmeryar\Inventory\Enums\Inventory\TrackingType::None->value)
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('type')
                    ->label(__('product.type'))
                    ->badge()
                    ->formatStateUsing(fn (\Jmeryar\Product\Enums\Products\ProductType $state): string => $state->label())
                    ->color(fn (\Jmeryar\Product\Enums\Products\ProductType $state): string => match ($state) {
                        \Jmeryar\Product\Enums\Products\ProductType::Product => 'primary',
                        \Jmeryar\Product\Enums\Products\ProductType::Service => 'success',
                        \Jmeryar\Product\Enums\Products\ProductType::Storable => 'warning',
                        \Jmeryar\Product\Enums\Products\ProductType::Consumable => 'info',
                    }),
        ...
        return [
            \Filament\Actions\Action::make('generate_variants')
                ...
                ->action(function (\Filament\Actions\Action $action, Product $record, $livewire) {
                    ...
                    $actionLogic->execute(new \Jmeryar\Product\DataTransferObjects\GenerateProductVariantsDTO(
