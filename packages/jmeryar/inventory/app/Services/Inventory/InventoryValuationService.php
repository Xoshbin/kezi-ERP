...
use Jmeryar\Inventory\DataTransferObjects\Inventory\CostDeterminationResult;
use Jmeryar\Inventory\Enums\Inventory\CostSource;
use Jmeryar\Inventory\Enums\Inventory\StockMoveType;
use Jmeryar\Inventory\Enums\Inventory\ValuationMethod;
use Jmeryar\Inventory\Exceptions\Inventory\InsufficientCostInformationException;
...
class InventoryValuationService
{
     ...
     *
     * @param  \Jmeryar\Accounting\Services\Accounting\LockDateService  $lockDateService  Service for enforcing accounting lock dates
     * @param  StockQuantService  $stockQuantService  Service for managing stock quantities
     * @param  \Jmeryar\Foundation\Services\CurrencyConverterService  $currencyConverter  Service for currency conversion
     */
    public function __construct(
        protected \Jmeryar\Accounting\Services\Accounting\LockDateService $lockDateService,
     ...
     * @throws \Jmeryar\Accounting\Exceptions\PeriodIsLockedException When transaction date is before lock date
     *
     * @example
    ...
    public function processIncomingStock(Product $product, float $quantity, Money $costPerUnit, Carbon $date, Model $sourceDocument): void
    {
        ...
        if ($sourceDocument instanceof \Jmeryar\Manufacturing\Models\ManufacturingOrder) {
     ...
     *
     * @throws \Jmeryar\Accounting\Exceptions\PeriodIsLockedException When adjustment date is before lock date
    ...
    public function createConsolidatedManualStockMoveJournalEntry(StockMove $stockMove): ?JournalEntry
    {
        ...
        foreach ($stockMove->productLines as $productLine) {
            ...
            if ($isOutgoing) {
                ...
                $isScrap = $productLine->toLocation?->type === \Jmeryar\Inventory\Enums\Inventory\StockLocationType::Scrap;
        ...
        $journalEntry = app(\Jmeryar\Accounting\Actions\Accounting\CreateJournalEntryAction::class)->execute($journalEntryDTO);
    ...
    public function calculateIncomingCostPerUnitEnhanced(
        ...
        if ($stockMove->source_type === 'Jmeryar\Purchase\Models\VendorBill') {
