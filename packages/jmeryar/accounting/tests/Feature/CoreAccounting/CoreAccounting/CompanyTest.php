...
test('a company with existing financial records cannot be deleted', function () {
    ...
    expect(fn () => $this->company->delete())
        ->toThrow(\Jmeryar\Foundation\Exceptions\DeletionNotAllowedException::class, 'Cannot delete a company with associated financial records.'); // <-- Corrected Message
...
test('duplicate tax ID for a company in the same fiscal country is prevented', function () {
    ...
    $companyService = app(\Jmeryar\Foundation\Services\CompanyService::class);
...
test('creating a currency with an existing code is prevented', function () {
    ...
    $currencyService = app(\Jmeryar\Foundation\Services\CurrencyService::class);
