...
use Jmeryar\Accounting\DataTransferObjects\Reports\CashFlowStatementDTO;
...
use Jmeryar\Accounting\Services\Reports\CashFlowStatementService;
...
test('it generates a cash flow statement with operating, investing, and financing activities', function () {
    ...
    $arAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::Receivable]);
    $apAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::Payable]);
    $fixedAssetAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::FixedAssets]);
    $equityAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::Equity]);
...
test('it adds back depreciation in operating activities', function () {
    ...
    $salesAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::Income]);
    $depreciationAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::Depreciation]);
    $accumulatedDepAccount = Account::factory()->for($this->company)->create(['type' => \Jmeryar\Accounting\Enums\Accounting\AccountType::FixedAssets]);
