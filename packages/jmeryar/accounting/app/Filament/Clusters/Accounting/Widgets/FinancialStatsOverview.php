...
use Jmeryar\Accounting\DataTransferObjects\Reports\BalanceSheetDTO;
...
class FinancialStatsOverview extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        // Get services
        $plService = app(\Jmeryar\Accounting\Services\Reports\ProfitAndLossStatementService::class);
        $bsService = app(\Jmeryar\Accounting\Services\Reports\BalanceSheetService::class);
        $arService = app(\Jmeryar\Accounting\Services\Reports\AgedReceivableService::class);
        $apService = app(\Jmeryar\Accounting\Services\Reports\AgedPayableService::class);
        ...
        return [
            ...
            Stat::make(__('accounting::dashboard.financial.current_month_profit'), \Jmeryar\Foundation\Support\NumberFormatter::formatMoneyTo($netProfit))
            ...
            Stat::make(__('accounting::dashboard.financial.ytd_profit'), \Jmeryar\Foundation\Support\NumberFormatter::formatMoneyTo($ytdNetProfit))
            ...
            Stat::make(__('accounting::dashboard.financial.total_receivables'), \Jmeryar\Foundation\Support\NumberFormatter::formatMoneyTo($totalReceivables))
            ...
            Stat::make(__('accounting::dashboard.financial.total_payables'), \Jmeryar\Foundation\Support\NumberFormatter::formatMoneyTo($totalPayables))
                ->description(__('accounting::dashboard.financial.outstanding_vendor_bills'))
                ->descriptionIcon('heroicon-m-credit-card')
                ->color($totalPayables->isZero() ? 'gray' : 'danger'),

            // Cash Balance
            Stat::make(__('accounting::dashboard.financial.cash_balance'), \Jmeryar\Foundation\Support\NumberFormatter::formatMoneyTo($cashBalance))
            ...
            // Gross Profit Margin
            Stat::make(__('accounting::dashboard.financial.gross_margin'), \Jmeryar\Foundation\Support\NumberFormatter::formatPercentage($grossProfitMargin, 1))
     ...
     */
    private function getMonthlyProfitTrend(Company $company, \Jmeryar\Accounting\Services\Reports\ProfitAndLossStatementService $plService): array
    {
        $data = [];
