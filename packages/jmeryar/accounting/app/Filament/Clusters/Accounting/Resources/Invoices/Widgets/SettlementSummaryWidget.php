...
namespace Jmeryar\Accounting\Filament\Clusters\Accounting\Resources\Invoices\Widgets;
...
class SettlementSummaryWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        if ($invoice->status !== InvoiceStatus::Posted) {
            return [
                Stat::make(__('accounting::invoice.settlement_widget.status'), __('accounting::invoice.settlement_widget.not_posted'))
                    ->description(__('accounting::invoice.settlement_widget.not_posted_desc'))
                    ->color('gray'),
            ];
        }
        ...
        return [
            Stat::make(__('accounting::invoice.settlement_widget.total_amount'), $this->formatMoney($totalAmount))
                ->description(__('accounting::invoice.settlement_widget.invoice_total'))
                ->color('info')
                ->icon('heroicon-o-document-text'),

            Stat::make(__('accounting::invoice.settlement_widget.paid_amount'), $this->formatMoney($paidAmount))
                ->description(__('accounting::invoice.settlement_widget.total_paid'))
                ->color('success')
                ->icon('heroicon-o-banknotes'),

            Stat::make(__('accounting::invoice.settlement_widget.outstanding_balance'), $this->formatMoney($outstandingBalance))
                ->description(__('accounting::invoice.settlement_widget.remaining_due'))
