...
class LockDateObserver
{
    public function updating(LockDate $lockDate): void
    {
        if ($lockDate->lock_type === LockDateType::HardLock) {
            throw new \Jmeryar\Foundation\Exceptions\UpdateNotAllowedException('A hard lock date cannot be modified.');
        }
    }
    ...
    {
        if ($lockDate->lock_type === LockDateType::HardLock) {
            throw new \Jmeryar\Foundation\Exceptions\UpdateNotAllowedException('A hard lock date cannot be removed.');
        }
    }
