...
namespace Jmeryar\Accounting\Listeners\Asset;
...
class CreateAssetFromVendorBillListener implements ShouldQueue
{
    ...
    public function handle(\Jmeryar\Purchase\Events\VendorBillConfirmed $event): void
    {
        $vendorBill = $event->vendorBill;
        /** @var \Jmeryar\Foundation\Models\Company $company */
        $company = $vendorBill->company;

        // Eager load relationships for efficiency
        $vendorBill->loadMissing('lines.expenseAccount', 'lines.product', 'currency', 'company.currency');

        /** @var \Jmeryar\Foundation\Models\Currency $companyCurrency */
        ...
        foreach ($vendorBill->lines as $line) {
            ...
            if ($line->asset_category_id) {
                $category = \Jmeryar\Accounting\Models\AssetCategory::find($line->asset_category_id);
            ...
            try {
                ...
                $assetDTO = new \Jmeryar\Accounting\DataTransferObjects\Assets\CreateAssetDTO(
