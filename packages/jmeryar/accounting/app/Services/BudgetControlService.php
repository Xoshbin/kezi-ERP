...
use Jmeryar\Accounting\Exceptions\BudgetExceededException;
use Jmeryar\Accounting\Models\Budget;
use Jmeryar\Accounting\Models\BudgetLine;
...
class BudgetControlService
{
    ...
    protected function getActuals(BudgetLine $line): Money
    {
        ...
        $query = \Jmeryar\Accounting\Models\JournalEntryLine::query()
    ...
    protected function getCommitted(BudgetLine $line): Money
    {
        ...
        $poQuery = \Jmeryar\Purchase\Models\PurchaseOrderLine::query()
            ->whereHas('purchaseOrder', function ($q) use ($line) {
                $q->where('company_id', $line->company_id)
                    ->whereIn('status', [
                        \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::Confirmed,
                        \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::ToReceive,
                        \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::ToBill,
                        \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::PartiallyReceived,
                        \Jmeryar\Purchase\Enums\Purchases\PurchaseOrderStatus::PartiallyBilled,
                    ])
        ...
        $billedQuery = \Jmeryar\Purchase\Models\VendorBillLine::query()
            ->whereHas('vendorBill', function ($q) use ($line) {
                $q->where('company_id', $line->company_id)
                    ->where('status', \Jmeryar\Purchase\Enums\Purchases\VendorBillStatus::Posted)
