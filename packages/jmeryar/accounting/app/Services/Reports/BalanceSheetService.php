...
class BalanceSheetService
{
    public function generate(Company $company, Carbon $asOfDate): BalanceSheetDTO
    {
        ...
        $assetLines = $this->mapBalancesToReportLines($accountBalances, \Jmeryar\Accounting\Enums\Accounting\AccountType::assetTypes(), $currency, $accounts);
        $liabilityLines = $this->mapBalancesToReportLines($accountBalances, \Jmeryar\Accounting\Enums\Accounting\AccountType::liabilityTypes(), $currency, $accounts, true);
        $equityLines = $this->mapBalancesToReportLines($accountBalances, \Jmeryar\Accounting\Enums\Accounting\AccountType::equityTypes(), $currency, $accounts, true);
    ...
    private function getAccountBalances(Company $company, Carbon $asOfDate): Collection
    {
        ...
        $results = DB::table('journal_entry_lines')
            ...
            ->whereIn('accounts.type', array_map(fn ($type) => $type->value, \Jmeryar\Accounting\Enums\Accounting\AccountType::balanceSheetTypes()))
    ...
    private function getCurrentYearEarnings(Company $company, Carbon $fiscalYearStart, Carbon $asOfDate): Money
    {
        ...
        $totalRevenue = Money::ofMinor(
            -$balances->get(\Jmeryar\Accounting\Enums\Accounting\AccountType::Income->value, 0) - $balances->get(\Jmeryar\Accounting\Enums\Accounting\AccountType::OtherIncome->value, 0),
            $company->currency->code
        );

        // Calculate total expenses (Expense accounts have debit nature)
        $totalExpenses = Money::ofMinor(
            $balances->get(\Jmeryar\Accounting\Enums\Accounting\AccountType::Expense->value, 0) +
                $balances->get(\Jmeryar\Accounting\Enums\Accounting\AccountType::Depreciation->value, 0) +
                $balances->get(\Jmeryar\Accounting\Enums\Accounting\AccountType::CostOfRevenue->value, 0),
            $company->currency->code
        );
    ...
    /**
     * @param  array<\Jmeryar\Accounting\Enums\Accounting\AccountType>  $types
