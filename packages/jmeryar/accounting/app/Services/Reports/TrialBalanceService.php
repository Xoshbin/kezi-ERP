...
use Jmeryar\Accounting\DataTransferObjects\Reports\TrialBalanceDTO;
use Jmeryar\Accounting\DataTransferObjects\Reports\TrialBalanceLineDTO;
    ...
    public function generate(Company $company, Carbon $asOfDate): TrialBalanceDTO
    {
        ...
        $reportLines = $results->map(function (object $row) use ($currency, $zero, &$totalDebit, &$totalCredit, $accounts) {
            ...
            return new TrialBalanceLineDTO(
                ...
                accountType: \Jmeryar\Accounting\Enums\Accounting\AccountType::from($row->account_type),
                debit: $debit,
                credit: $credit
            );
        });
