...
namespace Modules\Manufacturing\tests\Feature\Actions;
...
use Modules\Manufacturing\Actions\ScrapManufacturingAction;
...
class ScrapManufacturingActionTest extends TestCase
{
    ...
    public function test_it_can_scrap_components_and_generate_accounting_entries()
    {
        ...
        $productLine = \Modules\Inventory\Models\StockMoveProductLine::create([
            'company_id' => $this->company->id,
        ...
        $journalEntry = \Modules\Accounting\Models\JournalEntry::where('source_type', StockMove::class)
            ->where('source_id', $scrapMove->id)
            ->first();

        // Check StockMoveValuation
        $valuation = \Modules\Inventory\Models\StockMoveValuation::where('stock_move_id', $scrapMove->id)->first();
