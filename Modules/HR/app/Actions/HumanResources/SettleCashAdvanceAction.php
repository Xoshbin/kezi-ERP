...
class SettleCashAdvanceAction
{
    ...
    public function execute(
        ...
        DB::transaction(function () use ($cashAdvance, $settlementMethod, $bankAccountId, $user) {
            ...
            $journal = \Modules\Accounting\Models\Journal::where('company_id', $company->id)
                ->whereIn('type', ['miscellaneous', 'general'])
                ->first();

            if (! $journal) {
                // Fallback to any journal
                $journal = \Modules\Accounting\Models\Journal::where('company_id', $company->id)->first();
            }
            ...
            if ($balance->isPositive() && $settlementMethod === 'cash_return') {
                ...
                $paymentDTO = new CreatePaymentDTO(
                    ...
                    payment_type: \Modules\Payment\Enums\Payments\PaymentType::Inbound,
