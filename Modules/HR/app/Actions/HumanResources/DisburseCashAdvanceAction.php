...
class DisburseCashAdvanceAction
{
    ...
    public function execute(CashAdvance $cashAdvance, int $bankAccountId, User $user): void
    {
        DB::transaction(function () use ($cashAdvance, $bankAccountId, $user) {
            ...
            if (! $journalId) {
                $cashJournal = \Modules\Accounting\Models\Journal::where('company_id', $company->id)
                    ->whereIn('type', [
                        \Modules\Accounting\Enums\Accounting\JournalType::Cash,
                        \Modules\Accounting\Enums\Accounting\JournalType::Bank,
                    ])
                    ->first();
            ...
            $partner = \Modules\Foundation\Models\Partner::firstOrCreate(
                [
            ...
            $paymentDTO = new CreatePaymentDTO(
                ...
                payment_type: \Modules\Payment\Enums\Payments\PaymentType::Outbound,
