...
it('can create an invoice with deferred revenue dates on line items', function () {
    ...
    \Modules\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'sale']);
    \Modules\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'miscellaneous']); // Required for processing if triggered
...
it('creates deferred item when confirming invoice via UI', function () {
    ...
    $this->company->update([
        'default_sales_journal_id' => \Modules\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'sale'])->id,
    ...
    \Modules\Accounting\Models\Journal::factory()->create(['company_id' => $this->company->id, 'type' => 'miscellaneous']);
    ...
    $deferredItem = DeferredItem::where('source_type', \Modules\Sales\Models\InvoiceLine::class)
