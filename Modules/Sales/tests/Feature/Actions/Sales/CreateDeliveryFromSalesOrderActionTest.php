...
it('marks sales order as fully delivered when all items delivered', function () {
    ...
    $picking = \Modules\Inventory\Models\StockPicking::where('origin', 'Sales Order: '.$salesOrder->so_number)->first();
    expect($picking)->not->toBeNull();
    expect($picking->state)->toBe(\Modules\Inventory\Enums\Inventory\StockPickingState::Done);
});
