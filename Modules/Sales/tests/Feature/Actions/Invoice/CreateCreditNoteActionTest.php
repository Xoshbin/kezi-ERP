...
namespace Modules\Sales\Tests\Feature\Actions\Invoice;
...
use Modules\Sales\Actions\Invoice\CreateCreditNoteAction;
use Modules\Sales\DataTransferObjects\Invoice\CreateCreditNoteDTO;
use Modules\Sales\DataTransferObjects\Invoice\CreateCreditNoteLineDTO;
 ...
 * @property \Modules\Sales\Models\Invoice $invoice
 * @property \Modules\Product\Models\Product $product
 * @property \Modules\Accounting\Models\Account $salesDiscountAccount
 * @property \Modules\Accounting\Models\Account $taxAccount
 * @property \Modules\Accounting\Models\Account $arAccount
 * @property \Modules\Accounting\Models\Journal $salesJournal
 */
...
beforeEach(function () {
    ...
    $this->salesJournal = \Modules\Accounting\Models\Journal::factory()->for($this->company)->create(['name' => 'Sales Journal', 'type' => 'sale']);
...
it('creates stock moves when credit note is posted', function () {
    ...
    $stockMove = \Modules\Inventory\Models\StockMove::where('source_id', $creditNote->id)
        ->where('source_type', AdjustmentDocument::class)
        ->first();

    // Type hinting for PHPStan
    assert($stockMove instanceof \Modules\Inventory\Models\StockMove);

    expect($stockMove)->not->toBeNull()
        ->and($stockMove->productLines)->toHaveCount(1);

    $line = $stockMove->productLines->first();
    assert($line instanceof \Modules\Inventory\Models\StockMoveProductLine);
...
it('creates journal entry when credit note is posted', function () {
    ...
    $tax = \Modules\Accounting\Models\Tax::factory()->for($this->company)->create(['rate' => 0.10, 'tax_account_id' => $this->taxAccount->id]);
    ...
    $jeAction = app(\Modules\Accounting\Actions\Accounting\CreateJournalEntryForAdjustmentAction::class);
    ...
    assert($discountLine instanceof \Modules\Accounting\Models\JournalEntryLine);
    ...
    assert($taxLine instanceof \Modules\Accounting\Models\JournalEntryLine);
    ...
    assert($arLine instanceof \Modules\Accounting\Models\JournalEntryLine);
