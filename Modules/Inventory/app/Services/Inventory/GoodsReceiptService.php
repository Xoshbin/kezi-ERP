...
class GoodsReceiptService
{
    ...
    public function cancel(StockPicking $picking, User $user): void
    {
        ...
        foreach ($picking->stockMoves as $move) {
            $move->update(['status' => \Modules\Inventory\Enums\Inventory\StockMoveStatus::Cancelled]);
        }
    }
    ...
    {
        return \Modules\Inventory\Models\StockMoveProductLine::where('source_type', \Modules\Purchase\Models\PurchaseOrderLine::class)
            ->where('source_id', $purchaseOrderLineId)
            ->whereHas('stockMove', function ($query) {
                $query->where('status', \Modules\Inventory\Enums\Inventory\StockMoveStatus::Done);
            })
            ->sum('quantity');
    }
}
