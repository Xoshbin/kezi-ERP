    ...
    protected function getCommitted(BudgetLine $line): Money
    {
        ...
        $poQuery = \Modules\Purchase\Models\PurchaseOrderLine::query()
            ->whereHas('purchaseOrder', function ($q) use ($line) {
                $q->where('company_id', $line->company_id)
                    ->whereIn('status', [
                        \Modules\Purchase\Enums\Purchases\PurchaseOrderStatus::Confirmed,
                        \Modules\Purchase\Enums\Purchases\PurchaseOrderStatus::ToReceive,
                        \Modules\Purchase\Enums\Purchases\PurchaseOrderStatus::ToBill,
        ...
        $billedQuery = \Modules\Purchase\Models\VendorBillLine::query()
            ->whereHas('vendorBill', function ($q) use ($line) {
                $q->where('company_id', $line->company_id)
                    ->where('status', \Modules\Purchase\Enums\Purchases\VendorBillStatus::Posted)
