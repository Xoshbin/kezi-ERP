...
class BankStatementLineObserver
{
     ...
     */
    public function creating(\Modules\Accounting\Models\BankStatementLine $bankStatementLine): void
    {
        ...
        if (! $bankStatementLine->relationLoaded('bankStatement') && $bankStatementLine->bank_statement_id) {
            $bankStatement = \Modules\Accounting\Models\BankStatement::find($bankStatementLine->bank_statement_id);
            if ($bankStatement) {
