...
class AdjustmentDocumentsRelationManager extends RelationManager
{
    ...
    {
        return __('sales::invoice.adjustment_documents_relation_manager.title');
    }
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Section::make(__('sales::invoice.adjustment_documents_relation_manager.document_details'))
                    ->schema([
                        Select::make('company_id')
                            ->relationship('company', 'name')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.company'))
                            ->required()
                        ...
                        Select::make('currency_id')
                            ...
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.currency'))
                            ->required()
                            ->default(function (): ?int {
                                $owner = $this->getOwnerRecord();
                        ...
                        Select::make('type')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.type'))
                            ->options([
                        ...
                        DatePicker::make('date')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.date'))
                            ->required()
                            ->default(now()),

                        TextInput::make('reference_number')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.reference_number'))
                            ->required()
                            ->maxLength(255)
                            ->placeholder('e.g., CN-2024-001'),

                        Select::make('status')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.status'))
                            ->options([
                        ...
                        Textarea::make('reason')
                            ->label(__('sales::invoice.adjustment_documents_relation_manager.reason'))
                            ->required()
                            ->rows(3)
                            ->columnSpanFull()
                            ->placeholder(__('sales::invoice.adjustment_documents_relation_manager.reason_placeholder')),
                    ])
                    ->columns(2),
            ]);
    }
        ...
        return $table
            ...
            ->columns([
                ...
                MoneyColumn::make('total_amount')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.total_amount'))
                    ->sortable(),

                MoneyColumn::make('total_tax')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.total_tax'))
                    ->sortable(),
                ...
                TextColumn::make('journalEntry.id')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.journal_entry'))
                    ->placeholder(__('sales::invoice.adjustment_documents_relation_manager.no_journal_entry'))
                    ->toggleable(),

                TextColumn::make('posted_at')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.posted_at'))
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),

                TextColumn::make('created_at')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.created_at'))
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
            ])
            ->filters([
                SelectFilter::make('type')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.filter_type'))
                    ->options([
                        AdjustmentDocumentType::CreditNote->value => AdjustmentDocumentType::CreditNote->label(),
                        AdjustmentDocumentType::Miscellaneous->value => AdjustmentDocumentType::Miscellaneous->label(),
                    ]),
                ...
                SelectFilter::make('status')
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.filter_status'))
                    ->options([
                        AdjustmentDocumentStatus::Draft->value => AdjustmentDocumentStatus::Draft->label(),
                        AdjustmentDocumentStatus::Posted->value => AdjustmentDocumentStatus::Posted->label(),
                        AdjustmentDocumentStatus::Cancelled->value => AdjustmentDocumentStatus::Cancelled->label(),
            ...
            ->headerActions([
                CreateAction::make()
                    ->label(__('sales::invoice.adjustment_documents_relation_manager.create_adjustment'))
