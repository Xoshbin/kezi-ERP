...
class AgingAnalysisWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        if ($invoice->status !== InvoiceStatus::Posted || $invoice->paymentState === \Modules\Foundation\Enums\Shared\PaymentState::Paid) {
            return [
                Stat::make(__('sales::invoice.aging_widget.status'), __('sales::invoice.aging_widget.not_applicable'))
                    ->description(__('sales::invoice.aging_widget.not_applicable_desc'))
                    ->color('gray'),
            ];
        }
        ...
        return [
            Stat::make(__('sales::invoice.aging_widget.days_outstanding'), $daysOutstanding)
                ->description(__('sales::invoice.aging_widget.days_since_due'))
                ->color($this->getDaysOutstandingColor($daysOutstanding))
                ->icon('heroicon-o-clock'),

            Stat::make(__('sales::invoice.aging_widget.outstanding_amount'), $this->formatMoney($outstandingAmount))
                ->description(__('sales::invoice.aging_widget.remaining_balance'))
                ->color($this->getOutstandingAmountColor($daysOutstanding))
                ->icon('heroicon-o-currency-dollar'),

            Stat::make(__('sales::invoice.aging_widget.aging_bucket'), $agingBucket['label'])
                ->description(__('sales::invoice.aging_widget.aging_category'))
                ->color($agingBucket['color'])
                ->icon('heroicon-o-chart-bar'),

            Stat::make(__('sales::invoice.aging_widget.payment_progress'), $this->getPaymentProgress($invoice))
                ->description(__('sales::invoice.aging_widget.payment_status'))
    ...
    private function getAgingBucket(int $daysOutstanding): array
    {
        if ($daysOutstanding <= 0) {
            return [
                'label' => __('sales::invoice.aging_widget.current'),
                'color' => 'success',
            ];
        } elseif ($daysOutstanding <= 30) {
            return [
                'label' => __('sales::invoice.aging_widget.bucket_1_30'),
                'color' => 'warning',
            ];
        } elseif ($daysOutstanding <= 60) {
            return [
                'label' => __('sales::invoice.aging_widget.bucket_31_60'),
                'color' => 'danger',
            ];
        } elseif ($daysOutstanding <= 90) {
            return [
                'label' => __('sales::invoice.aging_widget.bucket_61_90'),
                'color' => 'danger',
            ];
        } else {
            return [
                'label' => __('sales::invoice.aging_widget.bucket_90_plus'),
                'color' => 'gray',
            ];
        }
    }
