...
it('converts asset purchase value to company currency', function () {
    ...
    // Create USD Currency
    $usd = \Modules\Foundation\Models\Currency::factory()->create([
        'code' => 'USD',
        'name' => 'US Dollar',
        'symbol' => '$',
    ]);
    ...
    $asset = \Modules\Accounting\Models\Asset::where('source_id', $vendorBill->id)->first();
...
it('does not create asset for zero value line', function () {
    ...
    $assets = \Modules\Accounting\Models\Asset::where('source_id', $vendorBill->id)->get();
