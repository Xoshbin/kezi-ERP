...
namespace Modules\Accounting\Tests\Feature\Services\Consolidation;
...
test('it identifies inter-company receivable and payable balances', function () {
    ...
    // Debug DB
    // dump(\Modules\Accounting\Models\JournalEntryLine::all()->toArray());
    // dump(\Modules\Accounting\Models\JournalEntry::where('is_posted', true)->count());
    // dump(\Modules\Accounting\Models\JournalEntry::first()->toArray());
    ...
    expect($first)->toBeInstanceOf(\Modules\Accounting\Models\JournalEntryLine::class);
});
...
function createTestJournalEntry(Company $company, Account $account, float $amount, Partner $partner)
{
    ...
    $je = JournalEntry::factory()->create([
        ...
        'state' => \Modules\Accounting\Enums\Accounting\JournalEntryState::Posted, // Correct column & Enum
