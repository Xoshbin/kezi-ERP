...
namespace Modules\Accounting\Tests\Feature\Filament\Clusters\Accounting\Resources\Invoices;
...
it('auto-creates vendor bill in company B when invoice to partner B is confirmed in company A', function () {
    ...
    $product = Product::factory()->create([
        'company_id' => $this->companyA->id,
        'income_account_id' => Account::factory()->create(['company_id' => $this->companyA->id, 'type' => \Modules\Accounting\Enums\Accounting\AccountType::Income])->id,
    ]);
    ...
    $partnerA = \Modules\Foundation\Models\Partner::factory()->create([
    ...
    \Modules\Accounting\Models\Account::factory()->create([
        'company_id' => $this->companyB->id,
    ...
    if ($invoice->fresh()->status !== \Modules\Sales\Enums\Sales\InvoiceStatus::Posted) {
        $test->assertNotified();
    }

    // 4. Assert Invoice Confirmed
    expect($invoice->fresh()->status)->toBe(\Modules\Sales\Enums\Sales\InvoiceStatus::Posted);
