...
namespace Modules\ProjectManagement\Tests\Feature\Services;
...
it('creates a customer sales invoice from project invoice', function () {
    ...
    // 3. Assert
    expect($salesInvoice)->toBeInstanceOf(\Modules\Sales\Models\Invoice::class)
...
it('prevents multiple invoicing', function () {
    $projectInvoice = ProjectInvoice::factory()->create([
        'company_id' => $this->company->id,
        'status' => 'invoiced',
        'invoice_id' => \Modules\Sales\Models\Invoice::factory()->create(['company_id' => $this->company->id])->id,
    ]);
