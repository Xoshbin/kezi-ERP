<div>
                ...
                @foreach($preview['issues'] as $issue)
                    <li>
                        ...
                        @if($t === 'ap_account_missing')
                            ...
                            <a href="{{ \App\Filament\Clusters\Settings\Resources\Companies\CompanyResource::getUrl('edit', ['record' => $bill->company_id]) }}" class="ml-2 underline" target="_blank">{{ __('posting_preview.links.open_company') }}</a>
                        @elseif($t === 'input_tax_missing')
                            <a href="{{ \App\Filament\Clusters\Settings\Resources\Accounts\AccountResource::getUrl() }}" class="ml-2 underline" target="_blank">{{ __('posting_preview.links.fix_input_tax') }}</a>
                        ...
                        @elseif($t === 'asset_category_invalid')
                            <a href="{{ \App\Filament\Clusters\Accounting\Resources\Assets\AssetResource::getUrl() }}" class="ml-2 underline" target="_blank">{{ __('posting_preview.links.open_assets') }}</a>
                        @endif
                    </li>
                @endforeach
            </ul>
                    ...
                    <tr class="border-b">
                        <td class="py-2 pr-2">
                            ...
                            @if(!empty($l['account_id']))
                                <a href="{{ \App\Filament\Clusters\Settings\Resources\Accounts\AccountResource::getUrl('edit', ['record' => $l['account_id']]) }}" class="text-primary-600 hover:underline" target="_blank" rel="noreferrer">
                                    {{ $label ?: $l['account_id'] }}
                                </a>
                            @else
                                {{ $label ?: '—' }}
    ...
    <div class="mt-3 flex items-center justify-between">
        <div>
            ...
            <a href="{{ \App\Filament\Clusters\Settings\Resources\Companies\CompanyResource::getUrl('edit', ['record' => $bill->company_id]) }}" class="underline text-gray-700 ml-4" target="_blank">{{ __('posting_preview.links.open_company') }}</a>
        </div>
